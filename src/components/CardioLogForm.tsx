...
export const CardioLogForm = ({ activityType, onSave, onClose }: CardioLogFormProps) => {
  ...
  const [intervals, setIntervals] = useState<{ distance: string; paceMin: string; paceSec: string; restSec: string; paceUnit: string; }[]>([{ distance: '', paceMin: '', paceSec: '', restSec: '', paceUnit: String(defaultPaceUnit) }]);
  ...
  const autoPaceContinuous = useMemo(() => {
    ...
    if (distM > 0 && durSec > 0 && !pace) return calculatePace(durSec, distM, defaultPaceUnit);
    return undefined;
  }, [distance, durationMin, durationSec, pace, isRunning, defaultPaceUnit]);

  const addInterval = () => setIntervals([...intervals, { distance: '', paceMin: '', paceSec: '', restSec: '', paceUnit: String(defaultPaceUnit) }]);
  ...
  const handleSave = async () => {
    ...
    if (mode === 'continuous') {
      ...
      const finalPace = paceSeconds !== undefined ? paceSeconds : (distM > 0 && durSec > 0 ? calculatePace(durSec, distM, isRunning ? 1000 : 100) : undefined);
      const result = await onSave({
        session_name: sessionName || (isRunning ? t('cardioLog.defaultRunName') : t('cardioLog.defaultSwimName')),
      ...
      const ivData: CardioInterval[] = intervals.map((iv, idx) => {
        ...
        return { interval_order: idx, distance_m: distM, pace_seconds_per_unit: paceSec > 0 ? paceSec : undefined, pace_unit_m: parseInt(iv.paceUnit) || defaultPaceUnit, rest_seconds: parseInt(iv.restSec) || undefined, duration_seconds: undefined };
      });
      const totalDist = ivData.reduce((sum, iv) => sum + iv.distance_m, 0);
      const avgPace = ivData.filter(iv => iv.pace_seconds_per_unit).length > 0
        ? ivData.reduce((sum, iv) => sum + (iv.pace_seconds_per_unit || 0), 0) / ivData.filter(iv => iv.pace_seconds_per_unit).length : undefined;
      const result = await onSave({
        session_name: sessionName || (isRunning ? t('cardioLog.defaultRunIntervalName') : t('cardioLog.defaultSwimIntervalName')),
        total_distance_m: totalDist, avg_pace_seconds_per_unit: avgPace,
      }, ivData);
      if (!result.error) onClose();
    }
    setSaving(false);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className={`rounded-2xl border ${accentBorder} ${accentBg} p-5 space-y-4`}>
      <div className="flex items-center justify-between">
        <h3 className={`text-base font-bold ${accentColor}`}>
          {isRunning ? t('cardioLog.registerRunning') : t('cardioLog.registerSwimming')}
        </h3>
        <Button variant="ghost" size="icon" onClick={onClose} className="h-8 w-8"><X className="w-4 h-4" /></Button>
      </div>

      <Input placeholder={t('cardioLog.sessionNamePlaceholder')} value={sessionName} onChange={e => setSessionName(e.target.value)} className="bg-background/50" />

      <div className="flex gap-2">
        <Button variant={mode === 'continuous' ? 'default' : 'outline'} size="sm" onClick={() => setMode('continuous')}
          className={mode === 'continuous' ? (isRunning ? 'bg-green-500 hover:bg-green-600' : 'bg-cyan-500 hover:bg-cyan-600') : ''}>
          <Route className="w-3.5 h-3.5 mr-1.5" />{t('cardioLog.continuous')}
        </Button>
        <Button variant={mode === 'intervals' ? 'default' : 'outline'} size="sm" onClick={() => setMode('intervals')}
          className={mode === 'intervals' ? (isRunning ? 'bg-green-500 hover:bg-green-600' : 'bg-cyan-500 hover:bg-cyan-600') : ''}>
          <Timer className="w-3.5 h-3.5 mr-1.5" />{t('cardioLog.intervals')}
        </Button>
      </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input placeholder={isRunning ? '5:30' : '1:45'} value={pace} onChange={e => setPace(e.target.value)} className="bg-background/50" />
            </div>
          </div>
          <div>
              ...
              <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/50 border border-border">
                ...
                <div className="text-[10px] text-muted-foreground">
                  <span className="font-medium text-foreground">
                    {t('cardioLog.calculatedPace')}: {formatPaceUtil(autoPaceContinuous)}/{isRunning ? 'km' : '100m'}
                  </span>
                  <span className="block">({formatPaceDetailed(autoPaceContinuous)}) · ({formatUnitLabel(defaultPaceUnit)})</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-3">
          <AnimatePresence>
            {intervals.map((iv, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="rounded-xl border border-border bg-background/50 p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-foreground">{t('cardioLog.seriesNumber', { num: idx + 1 })}</span>
                  {intervals.length > 1 && <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => removeInterval(idx)}><Trash2 className="w-3.5 h-3.5 text-destructive" /></Button>}
                </div>
          ...
          <Button variant="outline" size="sm" onClick={addInterval} className="w-full">
            <Plus className="w-3.5 h-3.5 mr-1.5" />{t('cardioLog.addSeries')}
          </Button>
        </div>
      )}

      <Button onClick={handleSave} disabled={saving} className={`w-full font-semibold ${isRunning ? 'bg-green-500 hover:bg-green-600' : 'bg-cyan-500 hover:bg-cyan-600'} text-white`}>
        {saving ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Save className="w-4 h-4 mr-2" />}
        {t('cardioLog.saveSession')}
      </Button>
    </motion.div>
  );
};