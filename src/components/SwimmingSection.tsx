      ...
      <motion.div variants={itemVariants}>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="p-3 rounded-xl bg-cyan-500/10"><Waves className="w-6 h-6 text-cyan-500" /></div>
            <div>
              <h2 className="text-xl font-bold text-foreground">{t('swimmingSection.title')}</h2>
              <p className="text-sm text-muted-foreground">{t('swimmingSection.sessionsAndLogs', { sessions: getTotalCompletions(), logs: sessions.length })}</p>
            </div>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => { setShowLogForm(!showLogForm); setShowStats(false); }} className={showLogForm ? 'bg-cyan-500 text-white hover:bg-cyan-600 border-cyan-500' : ''}>
              <Plus className="w-4 h-4 mr-1" />{t('swimmingSection.register')}
            </Button>
            <Button variant={showStats ? "default" : "outline"} size="sm" onClick={() => { setShowStats(!showStats); setShowLogForm(false); }} className={showStats ? "bg-cyan-500 hover:bg-cyan-600" : ""}>
      ...
      {sessions.length > 0 && !showStats && !selectedSession && (
        <motion.div variants={itemVariants} className="space-y-2">
          <h3 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
            <Gauge className="w-3.5 h-3.5 text-cyan-500" />{t('swimmingSection.recentLogs')}
          </h3>
          {sessions.slice(0, 3).map(s => (
            <div key={s.id} className="gradient-card rounded-xl p-3 border border-border flex items-center justify-between cursor-pointer hover:border-cyan-500/30 transition-colors" onClick={() => setSelectedSession(s.id)}>
              <div>
                <p className="text-sm font-medium text-foreground">{s.session_name || t('swimmingSection.sessionDefault')}</p>
              ...
              <div className="flex items-center gap-1.5">
                {s.intervals && s.intervals.length > 0 && <span className="text-[10px] px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-500 font-medium">{s.intervals.length} series</span>}
      ...
      {workouts.map((workout, index) => {
        ...
        return (
          <motion.div key={workout.id} variants={itemVariants} transition={{ delay: index * 0.1 }}>
            <Card className={`gradient-card border-border overflow-hidden transition-all ${isSelected ? 'ring-2 ring-primary' : ''}`} onClick={() => setSelectedWorkout(isSelected ? null : workout.id)}>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-sm font-medium text-foreground"><Target className="w-4 h-4 text-primary" />{t('swimmingSection.seriesLabel')}</div>
                  <ul className="space-y-1.5 pl-6">{workout.sets.map((set, i) => <li key={i} className="text-sm text-muted-foreground list-disc">{set}</li>)}</ul>
                </div>
                {lastCompletion && <p className="text-xs text-muted-foreground">{t('swimmingSection.lastCompleted')}: {format(new Date(lastCompletion.completed_at), "d 'de' MMMM", { locale: dateLoc })}</p>}
                <AnimatePresence>
                  {isSelected && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="pt-4 border-t border-border">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <label htmlFor={`complete-${workout.id}`} className="text-sm font-medium text-foreground cursor-pointer" onClick={(e) => e.stopPropagation()}>{t('swimmingSection.completedLabel')}</label>
                      </div>
      ...
      <AnimatePresence>
        {showConfirmation && (
          <motion.div className="fixed bottom-24 left-4 right-4 p-4 bg-success/20 border border-success/30 rounded-xl text-center z-50" initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}>
            <p className="text-sm text-success font-medium">{t('swimmingSection.swimmingRegistered')}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
